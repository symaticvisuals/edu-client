import { Box, IconButton, List, ListItem, ListItemSecondaryAction, ListItemText, makeStyles, Typography } from '@material-ui/core';
    ...
    <div>
      ...
      <Box mt={2} ml={2}>
        ...
        <Typography variant="caption">
          <i>Duration <span style={{ color: '#4C0098' }}>1hr 45min</span></i>
        </Typography>
      </Box>
      <Tabs
        value={value}
        ...
        <List style={{margin: 0, padding: 0}}>
          {[0, 1, 2].map((item, index) => {
            return <ListItem key={index}>
              <ListItemText
                primary={<Typography variant="h3" style={{marginBottom: '4px'}}>{`Chapter ${index + 1}: How to start With WD`}</Typography>}
                secondary={<Typography><i>50 Minutes by Anuj Garg <span style={{color: '#4C0098'}}>Available in 2 days</span>  </i></Typography>}
              />
